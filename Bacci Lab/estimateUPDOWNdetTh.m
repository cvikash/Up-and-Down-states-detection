function [S_comb,S_LFP,S_2,mu,sigma,thUP,thDOWN,GMmodel,L_t,phi,k_high,k] = estimateUPDOWNdetTh(LFPdata,sf,selectedBands,phaseMethod,bands,theta,nSTD_UP,nSTD_DOWN,fHigh,combFlag,rmsWin,smoothWin)
...
[b_high,a_high] = ellip(2,0.1,40,fHigh(1,:)./(0.5*sf));
...
theta = zeros(1,size(selectedBands,1));
for bb = 1:size(selectedBands,1)
...
try
    GMmodel = fitgmdist(S_comb(S_comb<=prctile(S_comb,95)),3,'Start','plus','Options',options);
catch err
    keyboard;
    %     continue;
end
