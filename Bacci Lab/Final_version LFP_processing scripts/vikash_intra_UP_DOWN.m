 % Written by Valentina Pasquel, Tomasso Fellin and Vikash Choudhary
 % vikash_intra_UP_DOWN.m takes whole-cell intracellular and Local Field
 % Potential as inputs and 
   clear all
   close all
   channel = [1 2];
...
%         continue; %#ok<BRKCONT>
    ...
    legend('Undertermined period', 'Up states', 'Down states')
    %% Pre-processing of LFP trace
    % Low-pass filtering of LFP
    LFPseries_mV = LFPseries_mV(boundaryWin+1:end);
...
% 
    ...
    figure(8)
    plot(phi_values,L_UP_DOWN/(std(L_UP_DOWN)))
    xlim([0 2*pi])
